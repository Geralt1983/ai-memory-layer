...
class ChatRequest(BaseModel):
    ...
    include_recent: int = Field(
        5, ge=0, le=20, description="Number of recent memories to include"
    )
    include_relevant: int = Field(
        5, ge=0, le=20, description="Number of relevant memories to include"
    )
...
class CleanupRequest(BaseModel):
    ...
    max_memories: Optional[int] = Field(
        5000, ge=1, le=100000, description="Maximum memories to keep"
    )
    max_age_days: Optional[int] = Field(
        90, ge=1, le=3650, description="Maximum age in days"
    )
    min_relevance: Optional[float] = Field(
        0.05, ge=0.0, le=1.0, description="Minimum relevance score"
    )
    archive_before_cleanup: bool = Field(
        True, description="Archive memories before deletion"
    )
...
class ExportRequest(BaseModel):
    ...
    format: str = Field("json", pattern="^(json|csv|txt)$", description="Export format")
