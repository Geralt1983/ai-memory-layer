...
class MemoryManagerDemo:
    ...
    def demonstrate_cleanup(self):
        ...
        old_memories = [
            memory for memory in self.engine.memories 
    ...
    def demonstrate_statistics(self):
        ...
        print(f"Average content length: {sum(len(m.content) for m in memories) / len(memories):.1f} chars")
        
        # Time distribution
        now = datetime.now()
        recent_count = len([m for m in memories if (now - m.timestamp).total_seconds() < 3600])
        old_count = len([m for m in memories if (now - m.timestamp).total_seconds() >= 86400])
