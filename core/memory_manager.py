...
class MetadataBasedCleanup(MemoryCleanupStrategy):
    ...
    def get_priority(self, memory: Memory) -> float:
        matches = sum(1 for key, value in self.criteria.items() 
...
class MemoryArchiver:
    ...
    def archive_memories(self, memories: List[Memory], criteria: Dict[str, Any]) -> ArchiveInfo:
        """Archive memories to compressed file"""
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        criteria_hash = hashlib.md5(str(sorted(criteria.items())).encode()).hexdigest()[:8]
        archive_filename = f"memories_{timestamp}_{criteria_hash}.json.gz"
        archive_path = self.archive_directory / archive_filename
        
        self.logger.info("Starting memory archival", extra={
        ...
        log_memory_operation("archive", memory_count=len(memories), 
                           archive_path=str(archive_path), size_bytes=size_bytes)
        
        self.logger.info("Memory archival completed", extra={
            "memory_count": len(memories),
            "archive_path": str(archive_path),
            "size_bytes": size_bytes,
            "compression_ratio": self._estimate_compression_ratio(memories, size_bytes)
        })
        
        return archive_info
    
    def _estimate_compression_ratio(self, memories: List[Memory], compressed_size: int) -> float:
        """Estimate compression ratio"""
        uncompressed_size = sum(len(json.dumps(memory.to_dict(), default=str)) 
    ...
    def load_archive(self, archive_path: str) -> List[Memory]:
        ...
        try:
            ...
            self.logger.info("Archive loaded successfully", extra={
            ...
            self.logger.error("Failed to load archive", extra={
                "archive_path": archive_path,
                "error": str(e)
            }, exc_info=True)
            raise
    ...
    def list_archives(self) -> List[ArchiveInfo]:
        ...
        for archive_file in self.archive_directory.glob("*.json.gz"):
            try:
                ...
                self.logger.warning("Failed to read archive metadata", extra={
                    "archive_path": str(archive_file),
                    "error": str(e)
                })
...
class MemoryManager:
    ...
    def __init__(self, memory_engine: MemoryEngine, archive_directory: str = "./archives"):
    ...
    def cleanup_memories(self, archive_before_cleanup: bool = True, 
        ...
        self.logger.info("Starting memory cleanup", extra={
        ...
        sorted_memories = sorted(self.memory_engine.memories, 
        ...
        if archive_before_cleanup and memories_to_cleanup and not dry_run:
            try:
                archive_info = self.archiver.archive_memories(
                    memories_to_cleanup,
                    {"cleanup_reason": "scheduled_cleanup", "strategies": [s.name for s in self.cleanup_strategies]}
                )
                archived_count = archive_info.memory_count
            except Exception as e:
                self.logger.error("Failed to archive memories before cleanup", extra={
                    "error": str(e)
                }, exc_info=True)
        ...
        log_memory_operation("cleanup", 
                           memories_before=initial_count,
                           memories_after=final_count,
                           memories_cleaned=cleaned_count,
                           memories_archived=archived_count)
        
        self.logger.info("Memory cleanup completed", extra={
            "memories_before": initial_count,
            "memories_after": final_count,
            "memories_cleaned": cleaned_count,
            "memories_archived": archived_count,
    ...
    def auto_cleanup(self, max_memories: int = 1000, max_age_days: int = 30,
                    min_relevance: float = 0.1) -> CleanupStats:
    ...
    def export_memories(self, output_path: str, format: str = "json",
        ...
        self.logger.info("Exporting memories", extra={
        ...
        if format.lower() == "json":
            with open(output_path, 'w') as f:
                json.dump([memory.to_dict() for memory in memories], f, indent=2, default=str)
        ...
        self.logger.info("Memory export completed", extra={
            "output_path": output_path,
            "format": format,
            "memory_count": len(memories)
        })
...
def create_default_memory_manager(memory_engine: MemoryEngine) -> MemoryManager:
    ...
    manager.add_cleanup_strategy(SizeBasedCleanup(max_memories=5000))  # Keep 5000 most recent
    manager.add_cleanup_strategy(AgeBasedCleanup(max_age_days=90))     # Remove memories older than 90 days
    manager.add_cleanup_strategy(RelevanceBasedCleanup(min_relevance=0.05))  # Remove very low relevance
    manager.add_cleanup_strategy(DuplicateCleanup(similarity_threshold=0.95))  # Remove duplicates
