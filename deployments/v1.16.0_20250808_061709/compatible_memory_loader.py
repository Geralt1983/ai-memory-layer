...
class CompatibleMemoryLoader:
    ...
    def load_chatgpt_memories_compatible(
        ...
        """
        logger.info("🚀 Starting compatible ChatGPT memory loading...")
    ...
    def enhanced_search_memories(
        ...
        """
        Enhanced search that uses importance weighting from metadata
        """
        ...
        try:
            ...
            query_embedding = memory_engine.embedding_provider.embed_text(query)
            if query_embedding is None:
                return []
            
            # Search with FAISS
            search_k = min(k * 3, len(memory_engine.memories))  # Get more for re-ranking
            similar_indices, similarities = memory_engine.vector_store.search(
            ...
            for idx, similarity in zip(similar_indices, similarities):
                if 0 <= idx < len(memory_engine.memories):
                    memory = memory_engine.memories[idx]
                    ...
                    # Combined score
                    final_score = base_score * importance * type_multiplier * age_factor
    ...
    def test_system(self, memory_engine: MemoryEngine):
        """Test the compatible memory system"""
        logger.info("🔍 Testing compatible memory system...")
...
def main():
    """Test the compatible loader"""
    if len(sys.argv) < 3:
        print("Usage: python compatible_memory_loader.py <memory.json> <faiss_index_path>")
        print("Example: python compatible_memory_loader.py data/chatgpt_memories.json data/faiss_chatgpt_index")
        sys.exit(1)
    ...
    try:
        ...
        if memory_engine and len(memory_engine.memories) > 0:
            logger.info(f"🎉 Compatible system ready with {len(memory_engine.memories)} memories!")
            ...
            logger.error("❌ Failed to create compatible memory system")
            sys.exit(1)
