    ...
    try:
        ...
        memories_to_add = [
            (
                ...
                {"type": "preference", "category": "programming"},
            ),
            (
                "The weather is nice today",
                {"type": "observation", "category": "weather"},
            ),
            (
                "FastAPI is great for building APIs",
                {"type": "fact", "category": "programming"},
            ),
        ...
        print(
            f"   Average content length: {memory_stats['average_content_length']:.1f}"
        )

        # Demonstrate cleanup (dry run)
        print("\n8. Testing cleanup (dry run)...")
        cleanup_result = client.cleanup_memories(
            max_memories=3, max_age_days=1, dry_run=True  # Very recent to show effect
        )
...
def interactive_mode():
    ...
    print(
        "Commands: chat, memory, search, recent, stats, clear, cleanup, archives, export, quit"
    )
    print("=" * 80)
        ...
        try:
            ...
            if command == "quit" or command == "exit":
                ...
                result = client.cleanup_memories(
                    max_memories=max_mem, max_age_days=max_age, dry_run=dry_run
                )
                ...
                format_choice = (
                    input("Format (json/csv/txt, default json): ").strip().lower()
                )
                ...
                try:
                    data = client.export_memories(
                        format=format_choice, filter_type=filter_type
                    )
                ...
                print(
                    "Unknown command. Try: chat, memory, search, recent, stats, clear, cleanup, archives, export, quit"
                )
