...
class TestMemoryCleanupStrategies:
    ...
    def test_age_based_cleanup(self):
        ...
        old_memory = Memory(
            content="Old memory", timestamp=datetime.now() - timedelta(days=45)
        )

        # Recent memory (should be kept)
        recent_memory = Memory(
            content="Recent memory", timestamp=datetime.now() - timedelta(days=15)
        )
    ...
    def test_relevance_based_cleanup(self):
        ...
        low_relevance_memory = Memory(content="Low relevance", relevance_score=0.2)

        # High relevance memory (should be kept)
        high_relevance_memory = Memory(content="High relevance", relevance_score=0.8)
...
class TestMemoryManager:
    ...
    def test_auto_cleanup(self, memory_engine, temp_archive_dir):
        ...
        stats = manager.auto_cleanup(max_memories=3, max_age_days=50, min_relevance=0.5)
    ...
    def test_export_memories_with_filter(self, memory_engine, temp_archive_dir):
        ...
        try:
            ...
            manager.export_memories(
                export_path, format="json", filter_func=high_relevance_filter
            )
...
class TestMemoryManagerAPI:
    ...
    def test_cleanup_endpoint_integration(self, mock_memory_manager):
        ...
        request = CleanupRequest(
            max_memories=1000, max_age_days=30, min_relevance=0.1, dry_run=False
        )
        ...
        mock_memory_manager.auto_cleanup.assert_called_once_with(
            max_memories=1000, max_age_days=30, min_relevance=0.1
        )
