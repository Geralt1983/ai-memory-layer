...
from core.memory_manager import (
    MemoryManager, create_default_memory_manager,
    AgeBasedCleanup, SizeBasedCleanup, RelevanceBasedCleanup, DuplicateCleanup
)
...
def create_sample_memories(count: int = 50) -> list:
    ...
    memory_types = ["user_message", "assistant_response", "system_note", "preference", "fact"]
...
def demonstrate_memory_manager():
    ...
    for archive in archives:
        print(f"  {Path(archive.archive_path).name}: {archive.memory_count} memories, {archive.size_bytes} bytes")
    ...
    for format_name in export_formats:
        ...
        try:
            ...
            print(f"  Exported to {format_name.upper()}: {export_path} ({file_size} bytes)")
    ...
    while True:
        ...
        if choice == 'q':
            ...
            for i, archive in enumerate(archives):
                ...
                print(f"     Memories: {archive.memory_count}, Size: {archive.size_bytes} bytes")
